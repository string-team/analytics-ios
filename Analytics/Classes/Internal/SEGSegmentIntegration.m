...
- (id)initWithAnalytics:(SEGAnalytics *)analytics httpClient:(SEGHTTPClient *)httpClient storage:(id<SEGStorage>)storage
{
    if (self = [super init]) {
        ...
        self.storage = storage;
			  NSURL *urlEndpoint = [NSURL URLWithString:self.configuration.endpoint];
        self.apiURL = [urlEndpoint URLByAppendingPathComponent:@"import"];
...
- (void)sendData:(NSArray *)batch
{
    ...
    self.batchRequest = [self.httpClient upload:payload forWriteKey:self.configuration.writeKey forEndpoint:self.configuration.endpoint completionHandler:^(BOOL retry) {
        [self dispatchBackground:^{
            if (retry) {
